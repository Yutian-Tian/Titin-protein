...
def save_average_curve(r_values, f_values, filename_prefix="average_curve", filter_limit=None):
    """
    保存平均值曲线数据到CSV文件
    ...
    if filter_limit is not None:
        filename = f"{filename_prefix}_f_filtered_{filter_limit}.csv"
    else:
        filename = f"{filename_prefix}_all_data.csv"
    
    # 保存到CSV
    save_path = '/home/tyt/project/Single-chain/opt+R/M-S_Rand_L+E/column_format/'
    full_path = save_path + filename
    
    df_avg.to_csv(full_path, index=False)
    print(f"Average curve saved to: {full_path}")
    print(f"  Number of data points: {len(df_avg)}")
    print(f"  r range: [{df_avg['r'].min():.4f}, {df_avg['r'].max():.4f}]")
    print(f"  f range: [{df_avg['f'].min():.4f}, {df_avg['f'].max():.4f}]")
...
def plot_f_r_curves(df1, df2, f_upper_limit=None, title="f-r Curves", save_figure=False):
    ...
    # ========== 保存完整平均值曲线数据 ==========
    # 保存完整平均值曲线数据（无论是否过滤）
    save_average_curve(r_uniform_full, f_avg_full, 
                      filename_prefix="average_curve",
                      filter_limit=None)
    ...
    if f_upper_limit is not None:
        ...
        if segments:
            ...
            for start_idx, end_idx in segments:
                segment_r = r_samples[start_idx:end_idx+1]
                segment_f = f_samples[start_idx:end_idx+1]
                all_segment_r.extend(segment_r)
                all_segment_f.extend(segment_f)
            
            # 转换为numpy数组并排序
            all_segment_r = np.array(all_segment_r)
            all_segment_f = np.array(all_segment_f)
            
            # 按r值排序
            sort_idx = np.argsort(all_segment_r)
            all_segment_r = all_segment_r[sort_idx]
            all_segment_f = all_segment_f[sort_idx]
            
            # 保存过滤后的平均值曲线
            save_average_curve(all_segment_r, all_segment_f,
                             filename_prefix="average_curve",
                             filter_limit=f_upper_limit)
    else:
        # 如果没有限制，直接绘制整个平均值曲线
