...
def main():
    f_file_path = '/home/tyt/project/Single-chain/opt+R/Multichain_results/f_multichain.csv'
    n_file_path = '/home/tyt/project/Single-chain/opt+R/Multichain_results/n_multichain.csv'
    
    # 2. Read CSV file
    df = pd.read_csv(f_file_path)
    dn = pd.read_csv(n_file_path)
    
    # 3. Calculate row average f_a (use first 100 columns)
    ...
    plt.plot(R[:820], f_a[:820], 'b-', linewidth=2)
    plt.xlabel('R', fontsize=12)
    plt.ylabel('$f_{a}$ (Row Average)', fontsize=12)
    plt.title('$f_{a}$ vs R Curve', fontsize=14)
    ...
    # 5. Plot with English labels
    plt.figure(figsize=(10, 6))
    plt.plot(R[:820], n_a[:820], 'b-', linewidth=2)
    plt.xlabel('R', fontsize=12)
