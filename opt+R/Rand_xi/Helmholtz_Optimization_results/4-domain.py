"""
目的：探究不同参数下的四个domain的解折叠顺序，参数包括xi_f1, beta2, beta3, beta4, E1, E2, E3, E4
系统：包含4个domain
主题：约束下优化系统的Helmholtz自由能
新增：可视化部分，绘制n1,n2,n3,n4和力曲线，风格与参考代码一致
"""
...
beta4 = 1.3          # xi_f4 / xi_f1
force_limit = 20.0   # 力曲线y轴上限
E0 = 2.0
delta1 = 0.0         # ΔE1 = E0 + delta1
delta2 = 0.1         # ΔE2 = E0 + delta2
delta3 = 0.2         # ΔE3 = E0 + delta3
delta4 = 0.3         # ΔE4 = E0 + delta4

# 优化参数
r_grids = 800        # 拉伸点数（维度增加后适当减少）
init_points = 5      # 初始每个维度网格点数（共5^4=625个组合，可接受）
refine_levels = 4    # 细化层级
refine_points = 5    # 细化时每个维度点数
tol = 1e-6

# 设置存储路径
save_path = "/home/tyt/project/Single-chain/opt+R/Rand_xi/Helmholtz_Optimization_results/4_domain_results"
...
def free_energy_4_domain(r, r1, r2, r3, n1, n2, n3, n4,
                         DeltaE1, DeltaE2, DeltaE3, DeltaE4,
                         xi_f1, xi_f2, xi_f3, xi_f4):
    r4 = r - r1 - r2 - r3
    if r4 < 0 or r4 > contour_length_Lci(n4, xi_f4):
        return 1e300
    ...
    energy4 = single_domain_free_energy(r4, n4, DeltaE4, xi_f4)
    total = energy1 + energy2 + energy3 + energy4
    if not np.isfinite(total):
        return 1e300
    return total

def Optimize_single_point(r, init_points=init_points,
                          refine_levels=refine_levels,
                          refine_points=refine_points,
                          tol=tol):
    """
    使用粗网格 + 局部细化优化给定拉伸 r 下的自由能。
    返回 [r, r1_opt, r2_opt, r3_opt, n1_opt, n2_opt, n3_opt, n4_opt]
    """
    # 固定参数
    DeltaE1 = E0 + delta1
    DeltaE2 = E0 + delta2
    ...
    for level in range(refine_levels + 1):
        ...
        n4_grid = np.linspace(n4_min, n4_max, N)
            ...
            for n2 in n2_grid:
                ...
                for n3 in n3_grid:
                    ...
                    for n4 in n4_grid:
                        ...
                        # 可行性检查：总拉伸不能超过四个域最大轮廓长度之和，也不能小于0
                        if r < 0 or r > Lc1 + Lc2 + Lc3 + Lc4:
                            continue

                        # 定义目标函数（优化 r1, r2, r3）
                        def obj(x):
                            r1v, r2v, r3v = x
                            return free_energy_4_domain(r, r1v, r2v, r3v,
                                                        n1, n2, n3, n4,
                                                        DeltaE1, DeltaE2, DeltaE3, DeltaE4,
                                                        xi_f1, xi_f2, xi_f3, xi_f4)

                        # 边界: r1 ∈ [0, Lc1], r2 ∈ [0, Lc2], r3 ∈ [0, Lc3]
                        bounds = Bounds([0.0, 0.0, 0.0], [Lc1, Lc2, Lc3])
                        # 线性约束: r1+r2+r3 >= r - Lc4  且  r1+r2+r3 <= r
                        cons = [
                            {'type': 'ineq', 'fun': lambda x: x[0] + x[1] + x[2] - (r - Lc4)},  # >=0
                            {'type': 'ineq', 'fun': lambda x: r - (x[0] + x[1] + x[2])}         # >=0
                        ]
                        # 初始猜测：均分
                        x0 = [min(Lc1, max(0.0, r/4)),
                              min(Lc2, max(0.0, r/4)),
                              min(Lc3, max(0.0, r/4))]
        ...
        if level_best_F == float('inf'):
            return np.array([r, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan])
        ...
        idx_n4 = np.argmin(np.abs(n4_grid - best_n4))
        ...
        left_n4 = n4_grid[max(0, idx_n4-1)]
        right_n4 = n4_grid[min(N-1, idx_n4+1)]

        n1_min, n1_max = left_n1, right_n1
        n2_min, n2_max = left_n2, right_n2
        n3_min, n3_max = left_n3, right_n3
        n4_min, n4_max = left_n4, right_n4

        # 检查是否达到精度
        if (n1_max - n1_min < tol) and (n2_max - n2_min < tol) and \
           (n3_max - n3_min < tol) and (n4_max - n4_min < tol):
            break

    return np.array([r, best_r1, best_r2, best_r3, best_n1, best_n2, best_n3, best_n4])
...
def plot_n_curves(ax, r, n1, n2, n3, n4, title):
    ...
    ax.plot(r, n4, color='magenta', linewidth=lines_linewidth, linestyle=':', label='$n_4$', zorder=3)
...
def plot_force_curves(ax, r, force1, force2, force3, force4, title):
    ...
    ax.plot(r, force4, color='magenta', linewidth=lines_linewidth, linestyle=':', label='$f_4$', zorder=3)
    ax.set_xlabel('$r$', fontsize=label_fontsize)
...
def main():
    ...
    for i, r in enumerate(r_vals):
        ...
        # 返回: [r, r1_opt, r2_opt, r3_opt, n1_opt, n2_opt, n3_opt, n4_opt]
        r_val, r1_opt, r2_opt, r3_opt, n1_opt, n2_opt, n3_opt, n4_opt = opt_result
        r4_opt = r_val - r1_opt - r2_opt - r3_opt if not np.isnan(r1_opt) else np.nan
        results.append([r_val, r1_opt, r2_opt, r3_opt, r4_opt, n1_opt, n2_opt, n3_opt, n4_opt])

        if (i + 1) % 100 == 0:
            print(f"已处理 {i+1} / {len(r_vals)} 个点")

    # 保存为 CSV 文件
    df = pd.DataFrame(results, columns=['r', 'r1', 'r2', 'r3', 'r4', 'n1', 'n2', 'n3', 'n4'])
    ...
    # 标题
    title = (f"$\\Delta E_1 = {E0+delta1:.1f},\\ \\Delta E_2 = {E0+delta2:.1f},\\ "
             f"\\Delta E_3 = {E0+delta3:.1f},\\ \\Delta E_4 = {E0+delta4:.1f}$\n"
