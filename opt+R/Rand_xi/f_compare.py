...
def load_and_process_data(r_file_path, f_file_path):
    """
    加载并处理数据
    
    参数:
    r_file_path: r_values.csv文件路径
    f_file_path: f_values.csv文件路径
    
    返回:
    r_common: 公共网格
    f_avg: 平均f值
    all_interpolated_f: 所有插值后的f数据
    num_groups: 组数
    """
    # Step 1: 读取数据
    print(f"正在读取数据...")
    print(f"r_values文件: {r_file_path}")
    print(f"f_values文件: {f_file_path}")
    ...
    if not os.path.exists(f_file_path):
        print(f"错误: f_values.csv文件不存在于 {f_file_path}")
        return None, None, None, None
    
    # 读取CSV文件
    try:
        r_data = pd.read_csv(r_file_path, header=None)
        f_data = pd.read_csv(f_file_path, header=None)
    except Exception as e:
        print(f"读取文件时出错: {e}")
        return None, None, None, None
    
    print(f"r_values数据形状: {r_data.shape}")
    print(f"f_values数据形状: {f_data.shape}")
    
    # 检查数据形状是否匹配
    if r_data.shape[1] != f_data.shape[1]:
        print("警告: r和f数据的列数不匹配!")
        # 如果r只有一列，f有多列，则使用同一列r数据
        if r_data.shape[1] == 1 and f_data.shape[1] > 1:
            print("r数据只有一列，将使用同一列r数据对应所有f列")
            # 复制r列以匹配f的列数
            r_data = pd.concat([r_data] * f_data.shape[1], axis=1, ignore_index=True)
        else:
            # 取最小列数
            min_cols = min(r_data.shape[1], f_data.shape[1])
            r_data = r_data.iloc[:, :min_cols]
            f_data = f_data.iloc[:, :min_cols]
    ...
    for i in range(num_groups):
        # 获取当前组的r和f数据，移除NaN值
        r_col = r_data.iloc[:, i].dropna().values
        f_col = f_data.iloc[:, i].dropna().values
        
        # 确保r和f长度相同
        min_len = min(len(r_col), len(f_col))
        if min_len == 0:
        ...
        f_sorted = f_col[sort_idx]
        
        # 创建插值函数
        try:
            interp_func = interp1d(r_sorted, f_sorted, kind='linear', 
                                 bounds_error=False, fill_value=np.nan)
            
            # 在公共网格上插值
            f_interp = interp_func(r_common)
            all_interpolated_f.append(f_interp)
            
        except Exception as e:
            print(f"  第{i+1}组: 插值失败 - {e}")
    
    if not all_interpolated_f:
        print("错误: 没有成功插值的数据!")
        return None, None, None, None
    
    # 将所有插值数据组合成数组
    all_interpolated_f = np.array(all_interpolated_f)
    print(f"\n成功插值{len(all_interpolated_f)}组数据")
    
    # 计算平均值（忽略NaN）
    print("计算平均值...")
    f_avg = np.nanmean(all_interpolated_f, axis=0)
    
    # 计算标准差（可选）
    f_std = np.nanstd(all_interpolated_f, axis=0)
    
    print("数据处理完成!")
    
    return r_common, f_avg, all_interpolated_f, num_groups

def PlotMS(r, L_c, lineType ):
    x = r/L_c
    x = np.asarray(x)
    L_c = np.asarray(L_c)
    ...
    if np.any(mask):
        x_masked = x[mask]
        result[mask] = 0.25 * L_c * x_masked**2 * (3.0 - 2.0 * x_masked)/(1.0 - x_masked)
...
def visualize_data(r_common, f_avg, all_interpolated_f, num_groups, save_path=None):
    """
    可视化数据
    ...
    # 绘制原始轨迹（半透明灰细线）
    for i in range(len(all_interpolated_f)):
        ax.plot(r_common, all_interpolated_f[i], 
                color='gray', alpha=0.3, linewidth=0.8, zorder=1)
    
    # 绘制平均曲线（红色粗线）
    ax.plot(r_common, f_avg, 
            color='red', linewidth=lines_linewidth, 
            label=f'Average of {num_groups} curves', zorder=3)
    PlotWLC(r_common, L_c=90.0, lineType='-')
    PlotWLC(r_common, L_c=180.0, lineType='--')
    
    # 设置标签和标题
    ax.set_xlabel('End-to-end distance $r$', fontsize=label_fontsize)
    ax.set_ylabel('Force $f$', fontsize=label_fontsize)
    ax.set_title(f'Force $f$ vs. distance $r$', fontsize=title_fontsize, pad=20)
...
def main():
    """主程序"""
    print("=" * 80)
    print("f-r关系数据分析和可视化程序")
    print("=" * 80)
    
    # ============ 在这里指定文件路径 ============
    # 请根据您的实际情况修改这些路径
    r_file_path = "/home/tyt/project/Single-chain/opt+R/Rand_xi/simulation_results/r_values.csv"  # 替换为您的r_values.csv文件路径
    f_file_path = "/home/tyt/project/Single-chain/opt+R/Rand_xi/simulation_results/f_values.csv"  # 替换为您的f_values.csv文件路径
    output_path = "/home/tyt/project/Single-chain/opt+R/Rand_xi/simulation_results/f_r_analysis.png"  # 替换为您想要保存的输出路径
    
    # Step 1: 检查文件路径
    print(f"指定的r_values.csv路径: {r_file_path}")
    print(f"指定的f_values.csv路径: {f_file_path}")
    print(f"输出文件路径: {output_path}")
    
    # Step 2-3: 加载和处理数据
    r_common, f_avg, all_interpolated_f, num_groups = load_and_process_data(r_file_path, f_file_path)
    ...
    # Step 4: 可视化
    fig = visualize_data(r_common, f_avg, all_interpolated_f, num_groups, save_path=output_path)
