...
# ==================== 文件路径 ====================
ffile_path = '/home/tyt/project/Single-chain/opt+force/2-state_comparation/results/f_values.csv'
rfile_path = '/home/tyt/project/Single-chain/opt+force/2-state_comparation/results/r_values.csv'
save_path = '/home/tyt/project/Single-chain/opt+force/2-state_comparation/results/compare_f_r_curves.png'

# ==================== Step 1: 从CSV文件中读取数据 ====================
print("Step 1: 从CSV文件中读取数据...")
try:
    # 读取r值数据，假设第一行是表头
    r_data = pd.read_csv(rfile_path, header=None)  # header=0表示第一行是表头
    
    # 读取f值数据，f_values.csv只有一列
    f_data = pd.read_csv(ffile_path, header=None)  # header=0表示第一行是表头
    ...
    # 检查f数据的形状
    print(f"  f数据形状: {f_data.shape}")
    if len(f_data.shape) == 1 or f_data.shape[1] == 1:
        print("  f_values.csv只有一列，所有轨迹共享相同的f值序列")
        f_values = f_data.iloc[:, 0].values if f_data.shape[1] == 1 else f_data.values
    else:
        print("  警告: f_values.csv有多列，将使用第一列")
        f_values = f_data.iloc[:, 0].values
...
print(f"  公共网格: 从0到{grid_max:.3f} (0.9 * xi_u), 共1000个点")
...
for col in range(num_trajectories):
    ...
    try:
        ...
        # 显示前几个轨迹的详细信息
        if col < 3:
            print(f"  轨迹 {col+1}: r范围=[{r_valid.min():.3f}, {r_valid.max():.3f}], "
                  f"f范围=[{f_valid.min():.3f}, {f_valid.max():.3f}], "
                  f"插值有效点={valid_points}")
...
# 为公共网格中的每个r值计算理论f值
f_theory = np.full_like(r_common, np.nan)  # 初始化为NaN
...
r_clean, f_theory_clean = clean_data(r_common, f_theory)
print("  理论曲线清理完成")
...
# 绘制原始轨迹的采样点（浅灰色点）
for col in range(min(num_trajectories, 50)):  # 只绘制前50条轨迹，避免图形过于拥挤
    r_raw = r_data.iloc[:, col].values
    f_raw = f_values
    
    valid_mask = ~np.isnan(r_raw)
    if np.sum(valid_mask) > 1:
        plt.scatter(r_raw[valid_mask], f_raw[valid_mask], 
                   color='gray', alpha=0.1, s=5, zorder=1, edgecolors='none')
...
# 绘制理论曲线fa-r（绿色）
plt.plot(r_clean, f_theory_clean, color='green', linewidth=3.0, 
...
plt.title('Force-extension Curves: Simulation vs Theory', fontsize=20, fontweight='bold')
...
valid_traj_save_path = '/home/tyt/project/Single-chain/opt+force/2-state_comparation/results/valid_trajectories_count.png'
plt.savefig(valid_traj_save_path, dpi=300, bbox_inches='tight', facecolor='white')
print(f"有效轨迹数量图已保存至: {valid_traj_save_path}")

# ==================== 创建第三个图形：显示前几条轨迹的原始数据 ====================
plt.figure(figsize=(12, 8))
num_to_plot = min(20, num_trajectories)  # 最多显示20条轨迹
...
for col in range(num_to_plot):
    ...
    if np.sum(valid_mask) > 1:
        plt.plot(r_raw[valid_mask], f_raw[valid_mask], 
                color=colors[col], alpha=0.7, linewidth=1, label=f'Trajectory {col+1}')

# 绘制平均曲线
plt.plot(r_common, f_avg, color='red', linewidth=3, 
         label='Simulation Mean', zorder=10)

plt.xlabel('End-to-end distance $r$', fontsize=16)
plt.ylabel('Force $f$', fontsize=16)
plt.title(f'First {num_to_plot} Trajectories (Raw Data)', fontsize=18)
plt.legend(fontsize=10, ncol=2, loc='upper left')
plt.xlim(0, grid_max)
plt.grid(True, alpha=0.3, linestyle='--')
plt.tight_layout()

trajectories_save_path = '/home/tyt/project/Single-chain/opt+force/2-state_comparation/results/individual_trajectories.png'
plt.savefig(trajectories_save_path, dpi=300, bbox_inches='tight', facecolor='white')
print(f"个体轨迹图已保存至: {trajectories_save_path}")
...
# 显示f值的统计信息
print(f"\nf值统计:")
print(f"  范围: [{np.min(f_values):.3f}, {np.max(f_values):.3f}]")
print(f"  均值: {np.mean(f_values):.3f}")

# 显示前几条轨迹的r值范围
print(f"\n前5条轨迹的r值范围:")
for col in range(min(5, num_trajectories)):
    r_raw = r_data.iloc[:, col].values
    valid_mask = ~np.isnan(r_raw)
    if np.sum(valid_mask) > 0:
        r_valid = r_raw[valid_mask]
        print(f"  轨迹 {col+1}: [{np.min(r_valid):.3f}, {np.max(r_valid):.3f}]")